...
bool indicators_update_step(keyboard_state_t *keyboard, uint8_t current_step)
{
    ...
    uint16_t red_intensity   = 230;
    uint16_t green_intensity = 1008;
    uint16_t blue_intensity  = 69;
    ...
    if (keyboard->led_state & (1 << 1)) { // caps_lock
        ...
        if(_caps_changed == false) {
            SET_PWM_DUTY_2(CAPS_LED, 0x0400);
            _caps_changed = true;
        }
    } else {
        if(_caps_changed == true) {
            SET_PWM_DUTY_2(CAPS_LED, 0);
            _caps_changed = false;
        }
    }
...
void indicators_pwm_set_all_columns(uint16_t intensity)
{
    ...
    //
    SET_PWM_DUTY_2(LED_PWM_C1, 0x400);
    SET_PWM_DUTY_2(LED_PWM_C2, 0x400);
    SET_PWM_DUTY_2(LED_PWM_C3, 0x400);
    SET_PWM_DUTY_2(LED_PWM_C4, 0x400);
    SET_PWM_DUTY_2(LED_PWM_C5, 0x400);
    SET_PWM_DUTY_2(LED_PWM_C6, 0x400);
    SET_PWM_DUTY_2(LED_PWM_C7, 0x400);
    SET_PWM_DUTY_2(LED_PWM_C8, 0x400);
    SET_PWM_DUTY_2(LED_PWM_C9, 0x400);
    SET_PWM_DUTY_2(LED_PWM_C10, 0x400);
    SET_PWM_DUTY_2(LED_PWM_C11, 0x400);
    SET_PWM_DUTY_2(LED_PWM_C12, 0x400);
    SET_PWM_DUTY_2(LED_PWM_C13, 0x400);
    SET_PWM_DUTY_2(LED_PWM_C14, 0x400);
}
...
void indicators_pwm_enable()
{
    ...
    PWM30CON = (uint8_t)(PWM_MODE_ENABLE_BIT | PWM_SS_BIT | PWM_CLK_DIV);
    PWM32CON = PWM_SS_BIT;
